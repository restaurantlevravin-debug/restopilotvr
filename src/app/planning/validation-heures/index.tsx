...
function Contenu() {
  ...
  const { creerValidationMois, obtenirValidationMensuelle, obtenirSyntheseHeures, ajouterCommentaire, validerMois } = useValidationHeures();
  ...
  const synthese = obtenirSyntheseHeures(mois, annee);
  const estGerant = utilisateurActif?.role === "GERANT";

  useEffect(() => { if (estGerant && !validation) void creerValidationMois(mois, annee); }, [annee, estGerant, mois, validation]);
  useEffect(() => { setRemarques(validation?.remarques ?? {}); setCommentaire(validation?.commentaire ?? ""); }, [validation?.commentaire, validation?.id, validation?.remarques]);
  ...
  async function enregistrerRemarques(afficherConfirmation = true) { if (!synthese || !estGerant) return; for (const ligne of synthese.lignes) { await ajouterCommentaire(mois, annee, remarques[ligne.utilisateurId] ?? "", ligne.utilisateurId); } await ajouterCommentaire(mois, annee, commentaire); if (afficherConfirmation) Alert.alert("Remarques enregistrées"); }
  function demanderValidation() { Alert.alert("Valider le mois ?", "Cette validation humaine figera les remarques du mois.", [{ text: "Annuler", style: "cancel" }, { text: "Valider", onPress: async () => { await enregistrerRemarques(false); if (await validerMois(mois, annee, commentaire)) Alert.alert("🎉 Mois validé par l’Empire"); } }]); }
  async function exporter() { if (!entrepriseActive || !validation || !synthese || !estGerant) return; try { await exporterHeuresPDF({ entreprise: entrepriseActive, validation, synthese, validateurNom: validateur?.nom }); } catch { Alert.alert("Export impossible", "Le mois doit être validé avant de générer le PDF."); } }
  ...
  return <SafeAreaView style={styles.ecran}><Stack.Screen options={{ headerShown: false }} /><View style={styles.entete}><Pressable onPress={() => router.back()}><Text style={styles.retour}>‹ Retour</Text></Pressable><Text style={styles.surtitre}>👑 VALIDATION DES HEURES</Text><Text style={styles.titre}>{MOIS[mois - 1]} {annee}</Text><View style={styles.navigation}><Pressable style={styles.boutonSecondaire} onPress={() => changerMois(-1)}><Text style={styles.boutonSecondaireTexte}>Mois précédent</Text></Pressable><Pressable style={styles.boutonSecondaire} onPress={() => changerMois(1)}><Text style={styles.boutonSecondaireTexte}>Mois suivant</Text></Pressable></View></View><ScrollView contentContainerStyle={styles.contenu}>
    {!synthese ? <View style={styles.carte}><Text style={styles.vide}>Aucun planning mensuel disponible pour cette période.</Text></View> : <>
      ...
      {synthese.lignes.map((ligne) => <View key={ligne.utilisateurId} style={styles.carte}><Text style={styles.nom}>{ligne.nom}</Text><Text style={styles.poste}>{ligne.poste || "Poste non renseigné"} · Contrat {ligne.contratHebdomadaire}h</Text><View style={styles.chiffres}><Text style={styles.chiffre}>Prévu\n{heures(ligne.heuresPrevuesMinutes)}</Text><Text style={styles.chiffre}>Réalisé\n{heures(ligne.heuresRealiseesMinutes)}</Text><Text style={styles.chiffre}>Écart\n{ligne.ecartMinutes >= 0 ? "+" : ""}{heures(ligne.ecartMinutes)}</Text><Text style={styles.chiffre}>Sup. détectées\n{heures(ligne.heuresSupplementairesMinutes)}</Text></View><Text style={styles.indicateur}>{indicateur(ligne.ecartMinutes)}</Text><TextInput editable={estGerant && validation?.statut !== "VALIDEE"} placeholder="Remarque du gérant" placeholderTextColor="#776F61" value={remarques[ligne.utilisateurId] ?? ligne.remarque ?? ""} onChangeText={(texte) => setRemarques((actuelles) => ({ ...actuelles, [ligne.utilisateurId]: texte }))} style={styles.input} /></View>)}
      {estGerant ? <TextInput editable={validation?.statut !== "VALIDEE"} multiline placeholder="Commentaire général éventuel" placeholderTextColor="#776F61" value={commentaire} onChangeText={setCommentaire} style={styles.input} /> : null}
      {validation?.statut === "VALIDEE" ? <View style={styles.validation}><Text style={styles.validationTitre}>🎉 Mois validé par l’Empire</Text><Text style={styles.validationTexte}>Par {validateur?.nom ?? "Gérant"} · {validation.dateValidation ? new Date(validation.dateValidation).toLocaleString("fr-FR") : "—"}</Text></View> : estGerant ? <><Pressable style={styles.boutonSecondaireLarge} onPress={() => void enregistrerRemarques()}><Text style={styles.boutonSecondaireTexte}>Enregistrer les remarques</Text></Pressable><Pressable style={styles.boutonPrincipal} onPress={demanderValidation}><Text style={styles.boutonPrincipalTexte}>Valider le mois</Text></Pressable></> : <View style={styles.carte}><Text style={styles.vide}>Consultation uniquement</Text></View>}
      {estGerant && validation?.statut === "VALIDEE" ? <Pressable style={styles.boutonExport} onPress={exporter}><Text style={styles.boutonExportTexte}>Générer le PDF comptable</Text></Pressable> : null}
    </>}
  </ScrollView></SafeAreaView>;
}

export default function ValidationHeuresScreen() { const { utilisateurActif, verifierPermission } = useUser(); if (!utilisateurActif) return <Redirect href="/" />; if (!verifierPermission("consultationValidationsHeures")) return <Redirect href="/" />; return <PointagePadProvider mode="GESTION_GERANT"><ValidationHeuresProvider><Contenu /></ValidationHeuresProvider></PointagePadProvider>; }
